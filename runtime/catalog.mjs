...
export const PLAN_CATALOG = [
  ...
  { f: 'uv', prefix: 'uv_',
    ...
    not: '纯几何验证阶段不必展开 UV；**别手动摆 UV** —— uv_smart_project / uv_unwrap / uv_pack',
  ...
  { f: 'sweep', prefix: 'sweep_',
    ...
    not: '等距重复阵列（履带/链节走参数化 Array 配方）；**别手接路径段** —— 先用 sweep_analyze 算弯折半径',
  ...
  { f: 'audit', prefix: 'audit_',
    ...
    not: '主观"像不像"（那走 qc_*）；**别目测代替体检** —— 数值门用 audit_*（加 summary_only:true 瘦身，判定不变）',
  ...
  { f: 'qc', prefix: 'qc_',
    ...
    not: '能数值判定的装配问题（audit_* 更硬）；**别拿"看起来像"当验收** —— 用 qc_render_views(ref_path=…) 算 IoU / 剖面差',
  ...
  { f: 'shape', prefix: 'shape_',
    ...
    not: '不是新几何引擎：几何复用 img_*/vehicle_* 的量具与放样；也不适用于没外轮廓的（布料/毛发/流体）；**别手算站表/别自造截面** —— 截面通路先问 shape_plan(object_class=…) 的 section_path',
  ...
  { f: 'vehicle', prefix: 'vehicle_',
    ...
    not: '不做细节件（灯/格栅/后视镜/玻璃分件）—— 那些用硬表面单独做再挂；也不是车漆材质（material_*）；**别自己写参数化放样** —— 自造只能出一张连续光滑面（没棱没缝的肥皂）；车壳一律 vehicle_sections → vehicle_loft → vehicle_panels',
  ...
  { f: 'human', prefix: 'human_',
    ...
    not: '不做写实人体：脸/手这类高细节部位用面罩手套回避；也不是骨骼绑定（那是 motion_*）；**别自造人体比例** —— human_* 素体/头型起步',
  ...
  { f: 'img', prefix: 'img_',
    ...
    not: '不是"看图"本身（看图仍要 read_image/rt_see）；它是量具，负责把像素变成数字；**别用眼睛估尺寸** —— 像素变数字用 img_* 量具（比例/距离/IoU 都要数）',
  ...
  { f: 'gate', prefix: 'gate_',
    ...
    not: '不是单点体检（只查一项用 audit_mesh / print_report）；也不做渲染对照（那是 qc_*）；**别自写验收判据** —— gate_plan → gate_run，顶层读 verdict 三态',
  ...
  { f: 'material', prefix: 'material_',
    ...
    not: '只是换个纯色（改 BSDF 默认值就行）；也不是贴图库（那走 PolyHaven 集成）；**别手搭节点树** —— material_build(preset=…) 起步',
  ...
  { f: 'deliver', prefix: 'deliver_',
    ...
    not: '中间产物（直接 rt_headless 里 export 就行）；**别手写导出/别只交 .blend** —— deliver_export → deliver_verify（manifest md5）',
  ...
  { f: 'motion', prefix: 'motion_',
    ...
    not: '静态几何（audit_measure 就够）；**别手摆姿态** —— motion_joints → motion_measure → motion_export_urdf',
  ...
  { f: 'generator', prefix: 'generator_',
    ...
    not: '一次性造型；**别写一次性循环** —— 要能改参就 generator_save → generator_run',
