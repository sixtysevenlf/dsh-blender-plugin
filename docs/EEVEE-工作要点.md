...
> 状态更新（2026-09-14）：该 material_pipeline.md 所描述的第三方插件（blender_Stylized-Shading-Tools）**已删除且不再提供**。
> 但其中记录的**原理与坑**（主节点/光源拓展 BSDF 分工、Lightgroup ID 规则、Shader to RGB 只含直接光、渐变组绑定与映射错位诊断）
> 对**手工搭建的 NPR 节点组同样适用**，因此本节保留。
