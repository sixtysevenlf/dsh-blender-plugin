...
export function frameDedupe(key, hash, force = false) {
...
/** 测试用：清空去重缓存 */
export function resetFrameCache() { FRAME_CACHE.clear(); }
    ...
    ctx.effect(() => ctx.tools.register(vTool({
        ...
        async execute(args) {
            ...
            if (frm || look) {
                ...
                if (args && args.diagnostics !== undefined)
                    spec.diagnostics = !!args.diagnostics;
