...
export function apply(ctx, config) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        description: '【无头 Blender】独立进程跑脚本（blender.exe -b）：不占 GUI 通道、不动你正看着的场景，适合重渲染 / 批量几何 / 数据校验 / 长任务。脚本里 print("HEADLESS {...}") 会被解析成 result（回传 JSON 用）。默认 --factory-startup（干净、快、不会去抢 9876 端口）；要用用户的启动文件与插件时设 factory_startup=false。返回 stdout/stderr 尾部 + outdir 里的新文件。',
        parameters: {
            ...
            preload: { type: 'string', description: '预载 runtime 里的 python 模块（逗号分隔，如 "view,perf"）：源码拼到脚本开头，之后可用 K.dsh_view_api / K.dsh_perf_api —— 无头进程也能自定义视角出图、跑性能预设' },
        },
        output: { schema: ANY_SCHEMA, render: renderOne },
        isConcurrencySafe: () => false,
        async execute(args) {
