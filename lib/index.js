...
async function backendPost(port, path, body, timeoutMs = 60000) {
    ...
    try {
        ...
        j = { ok: false, raw: text.slice(0, 500) };
    }
    j.__status = r.status;
    return j;
}
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        name: 'blender_rt_do',
        description: '实时驱动 Blender：在 Blender 的 Python 里执行一段代码，可选立刻回一帧视口（整步约 105ms/次，可在同一轮里连续做几十步）。' + HINT_KERNEL + ' 返回值含 stdout 与耗时。',
        parameters: {
            code: { type: 'string', required: true, description: 'Python 代码（在 Blender 主线程执行；预置 bpy/math/mathutils/Vector 与持久内核 K）' },
        ...
        async execute(args) {
            ...
            const r = await backendPost(port, '/act', { code: code }, 180000);
            ...
            if (r && r.ok && r.stdout)
                parts.push('stdout: ' + String(r.stdout).trim().slice(0, 1500));
            let image = undefined;
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        description: '【无头 Blender】独立进程跑脚本（blender.exe -b）：不占 GUI 通道、不动你正看着的场景，适合重渲染 / 批量几何 / 数据校验 / 长任务。'
            ...
            + '⚠️ 两条必读语义：① 无头进程默认读不到用户偏好，Cycles 会**静默回落 CPU**（实测 15.4×）—— 本工具默认注入 GPU 前导（gpu:"auto"）并回传 gpu 字段；'
        ...
        parameters: {
            ...
            gpu: { type: 'string', description: 'GPU 语义：auto（默认，没有 GPU 后端就按 OPTIX→CUDA→HIP→ONEAPI→METAL 配好并启用设备）/ true（要求必须有 GPU，否则 ok=false）/ false（不做任何配置）' },
        ...
        async execute(args) {
            ...
            if (args && args.args)
                body.args = String(args.args).split(/\s+/).filter(Boolean);
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            gpu: { type: 'string', description: 'op=start 时的 GPU 语义：auto（默认）/ true / false' },
        },
        output: { schema: ANY_SCHEMA, render: renderOne },
        isConcurrencySafe: () => false,
        async execute(args) {
            ...
            if (lt)
                return { text: lt };
            if (!r || r.ok !== true)
                return { text: 'WORKER ' + op + ' 失败 · ' + String((r && (r.error || r.raw)) || 'unknown') + (r && r.hint ? ('\n' + String(r.hint)) : '') };
            const res = (r && r.result) || {};
            if (op === 'exec') {
                const parts = [];
