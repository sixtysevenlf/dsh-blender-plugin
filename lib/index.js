...
const vTool = (spec) => defineTool({ ...spec, description: '[v' + PLUGIN_VERSION + '] ' + String((spec && spec.description) || '') });
...
export function apply(ctx, config) {
    ...
    ctx.effect(() => ctx.tools.register(vTool({
        ...
        async execute(args) {
            ...
            if (args && args.args)
                body.args = splitArgs(args.args); // v0.8.9：引号成对时不当分隔符（旧版把 --flag="a b" 切碎）
            ...
            try {
                ...
                let runs = [];
                try {
                    const st = await backendJson(port, '/status', 10000);
                    runs = (st && st.runs) || [];
                }
                catch (e2) { /* ignore */ }
                const recent = runs.slice(-3).reverse().map((x) => '  · ' + String(x.id) + ' · ' + String(x.status) + ' · pid=' + String(x.pid)
            ...
            if (lt)
                return { text: lt };
            if (!r || r.error)
                return { text: 'HEADLESS 失败 · ' + String((r && (r.error || r.raw)) || 'unknown') + (r && r.hint ? ('\n' + String(r.hint)) : '') };
            ...
            if (!r.result) {
                ...
                if (Number(r.__heartbeats) > 0)
                    lines.push('（流式回执：' + String(r.__heartbeats) + ' 条心跳，但没取到结果行）');
                if (raw)
            ...
            if (res.status)
                parts.push('状态：' + String(res.status) + (res.failure_hint ? (' —— ' + String(res.failure_hint)) : ''));
            if (res.session)
                parts.push('会话：' + String(res.session));
            if (res.script)
                parts.push('脚本：' + String(res.script));
            if (res.scriptFile)
                parts.push('脚本文件：' + String(res.scriptFile.resolved || res.scriptFile.given) + '（' + String(res.scriptFile.bytes) + ' 字节'
                    + (res.scriptFile.auto_from_file ? '，由 file= 自动识别为脚本' : '') + '）');
            if (res.lastException)
                parts.push('最后异常：' + String(res.lastException));
            if (res.logs)
                parts.push('日志：' + String(res.logs.stdout) + ' · ' + String(res.logs.stderr));
            ...
            if (res.resultPath)
                parts.push('result 已落盘：' + String(res.resultPath) + '（' + String(res.resultBytes) + ' 字节，全量 JSON —— 不必再从 stdout 里 indexOf 切片）');
            if (res.childEnvKeys && res.childEnvKeys.length)
                parts.push('注入 env：' + res.childEnvKeys.join(', '));
            // v0.8.10（A3）：长输出「头 1.5KB + 尾 2.5KB + 全量落盘路径」（旧版只给尾部，长 JSON 的头被吃掉）
            const clipDisplay = (s, head, tail) => {
            ...
            if (res.stdout)
                parts.push((res.stdoutTruncated ? '--- stdout（头+尾，已截断）---' : '--- stdout ---') + '\n'
                    + (res.result ? clipDisplay(res.stdout, 500, 800) : clipDisplay(res.stdout, 1500, 2500)));
            if (res.stderr && String(res.stderr).trim())
                parts.push('--- stderr（头+尾）---\n' + clipDisplay(res.stderr, 500, 1500));
            if (res.runId)
                parts.push('运行号：' + String(res.runId) + '（blender_rt_job(op="status"/"collect", id=…" 可查；后端重启后也能从磁盘台账查到）');
            if (res.expect) {
                const ex = res.expect;
            ...
            if (res.artifacts && res.artifacts.length)
                parts.push('产物：' + res.artifacts.slice(0, 12).map((a) => a.name + '(' + a.bytes + 'B' + (a.fresh ? ' 新' : '') + ')').join(' · '));
            if (res.noiseFiltered)
                parts.push('（已过滤 ' + String(res.noiseFiltered) + ' 个噪音产物：__pycache__/*.pyc/*.blend1|2/tmp*）');
            if (res.hint)
                parts.push('提示：' + String(res.hint));
            if (res.traceback)
                parts.push('--- traceback ---' + String.fromCharCode(10) + String(res.traceback).slice(0, 1200));
            // v0.8.9：两条边界提示 —— ① 本次走了流式回执（>15 s，心跳 N 条）；② 长活下次直接转作业层
            const beats = Number((r && r.__heartbeats) || 0);
            if (beats > 0)
                parts.push('（本次耗时 > 15 s，后端走了流式回执：' + String(beats) + ' 条心跳后收到结果 —— v0.8.9 起客户端按行取末条，不再出现 exit=undefined）');
    ...
    ctx.effect(() => ctx.tools.register(vTool({
        ...
        async execute(args) {
            ...
            if (op === 'start')
                parts.push('已后台化（pid ' + String(j.pid) + '）。用 op=status/collect id=' + String(j.id) + ' 跟进；日志目录：' + String(j.logDir || ''));
            if (j.exitCode !== undefined && j.exitCode !== null)
                parts.push('exit=' + String(j.exitCode) + ' · ' + String(j.ms) + 'ms');
            if (j.result)
                parts.push('result: ' + JSON.stringify(j.result).slice(0, 1500));
            if (j.lastException)
                parts.push('lastException: ' + String(j.lastException));
            if ((j.artifacts || []).length)
                parts.push('产物（' + String(j.artifacts.length) + '）：' + j.artifacts.slice(0, 10).map((a) => a.name + '(' + a.bytes + 'B)').join(' · '));
            if (j.stdoutTail)
                parts.push('--- stdout 尾 ---' + String.fromCharCode(10) + String(j.stdoutTail).slice(-2000));
            if (j.stderrTail && String(j.stderrTail).trim())
                parts.push('--- stderr 尾 ---' + String.fromCharCode(10) + String(j.stderrTail).slice(-1200));
            return { text: parts.join(String.fromCharCode(10)) };
        },
